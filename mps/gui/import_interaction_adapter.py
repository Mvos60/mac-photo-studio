...
class PendingImportInteraction:
    ...
    _response: ImportResponse | None = field(default=None, init=False)

    def respond(self, response: ImportResponse) -> None:
        if not isinstance(response, ImportResponse):
            raise TypeError("Import interaction requires ImportResponse")
        with self._lock:
            if self._response is None:
                self._response = response
                self._ready.set()

    def wait(self, cancellation: Event) -> ImportResponse:
